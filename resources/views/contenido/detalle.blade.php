...
<div class="DetalleContenido">
    ...
    <div class="DetalleInfo">
        ...
        {{-- EL CONTENEDOR DE MENSAJES DE SESIÓN HA SIDO ELIMINADO AQUÍ --}}
        ...
        <div class="DetalleNombre card">
            ...
            <div class="descripcionContenido">
                ...
                {{-- *** INICIO DE BLOQUE DE BOTONES Y FORMULARIOS CORREGIDO *** --}}
                <div class="DetalleInfoButton" style="display: flex; gap: 10px; align-items: center;">
                    
                    {{-- 1. FORMULARIO PARA AGREGAR AL CARRITO + Selector de Cantidad --}}
                    <form id="form-carrito" action="{{ route('carrito+') }}" method="POST">
                        @csrf
                        {{-- Hidden inputs necesarios para agregar al carrito --}}
                        ...
                        <button class="btn btn-primary" type="submit" style="margin-right: 10px;">Agregar al carrito</button>
                        
                        {{-- Contenedor del combobox de unidades disponibles --}}
                        <div class="qty-dropdown-container" id="qty-selector-container" style="display: inline-block;"> 
                            ...
                            <!-- Menu desplegable -->
                            <div class="qty-dropdown-menu" id="qty-dropdown-menu" style="position: absolute; border: 1px solid #ccc; background: white; z-index: 10; display: none;">
                                ...
                                <div class="qty-custom-input-group" id="custom-qty-group" style="display: none; padding: 5px; border-top: 1px solid #eee;">
                                    <input type="number" id="custom-qty-value" placeholder="Cantidad" min="1" value="6" style="width: 70px;">
                    ...
                    {{-- 2. FORMULARIO PARA COMPRA DIRECTA --}}
                    <form id="form-compra-directa" action="{{ route('compra.directa') }}" method="POST" class="inline-block"
                          onsubmit="return confirm('¿Deseas comprar este producto ahora? Se generará un pedido de inmediato.');">
                        @csrf
                        
                        {{-- Input para el ID del producto --}}
                        <input type="hidden" name="id_producto" value="{{ $producto->id_producto }}">
                        
                        {{-- Input para la cantidad, sincronizado por JS --}}
                        <input type="hidden" name="cantidad" id="compra-directa-cantidad" value="1"> 
                        
                        <button class="btn btn-primary" type="submit">Comprar ahora</button>
                    </form>

                </div>
                {{-- *** FIN DE BLOQUE DE BOTONES Y FORMULARIOS CORREGIDO *** --}}
...
<script>
    ...
    const productStock = productStockElement ? parseInt(productStockElement.textContent) : 999; 
    
    // Inputs de formularios
    const finalQuantityInput = document.getElementById('final-cantidad'); 
    const compraDirectaCantidadInput = document.getElementById('compra-directa-cantidad'); // Input para compra directa

    // Función principal para actualizar la cantidad seleccionada
    function updateSelectedQuantity(qty) {
        ...
        // 1. Actualiza el input del carrito
        finalQuantityInput.value = finalQty; 
        currentQtySpan.textContent = finalQty;
        
        // 2. SINCRONIZA el input de Compra Directa
        if (compraDirectaCantidadInput) {
            compraDirectaCantidadInput.value = finalQty;
        }

        // Ocultar el menú después de la selección
        if (qtyMenu) qtyMenu.style.display = 'none';
        if (customGroup) customGroup.style.display = 'none';
        if (customTrigger) customTrigger.style.display = 'block';
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (qtyHeader) {
            ...
            qtyHeader.addEventListener('click', () => {
                const isVisible = qtyMenu.style.display === 'block';
                if (qtyMenu) qtyMenu.style.display = isVisible ? 'none' : 'block';
        ...
        if (customApplyBtn && customInput) {
            customApplyBtn.addEventListener('click', () => {
                ...
                if (isNaN(qty) || qty < 1) {
                    // En un entorno real usarías un modal o mensaje inline en lugar de alert()
                    console.error("Ingresa una cantidad válida."); 
                    return;
                }
                updateSelectedQuantity(qty);
            });
        ...
        document.addEventListener('click', (e) => {
            if (qtyContainer && !qtyContainer.contains(e.target)) {
                if (qtyMenu) qtyMenu.style.display = 'none';
                if (customGroup) customGroup.style.display = 'none';
                if (customTrigger) customTrigger.style.display = 'block';
            }
        ...
        // Inicializar la cantidad al cargar (si el stock lo permite)
        if (productStock < 6 && customTrigger) {
